...
//       logical predecessor and the predecessor's node is deleted.
{
  ...
  {
      TreeNode* node = PtrToSuccessor(tree->right);
      tree->info = node->info;
      data = node->info;
...
//Recursive version
TreeNode* TreeType::PtrToSuccessor_A(TreeNode* &tree)
{
    if(tree->left != NULL)
        return PtrToSuccessor_A(tree->left);
    else
    {
        TreeNode* tempPtr = tree;
        tree = tree->right;
...
TreeNode* TreeType::PtrToSuccessor_B(TreeNode *tree)
{
